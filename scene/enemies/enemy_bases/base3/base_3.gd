...
@export var sputnik_scene: PackedScene
var coin_count = 15
var brilliant_count = 8
...
func spawn_sputniks():
	var sputnik = sputnik_scene.instantiate()
	sputnik.global_position = global_position + Vector2(300, -400)
	get_parent().add_child(sputnik)
	
	sputnik = sputnik_scene.instantiate()
	sputnik.global_position = global_position + Vector2(-300, -400)
	get_parent().add_child(sputnik)
