...
var damage = randi_range(150,200)
...
func _ready():
	if has_node("CollisionShape2DBoom"):
		$CollisionShape2DBoom.set_deferred("disabled", true)
...
func boom():
	if has_node("CollisionShape2DBoom"):
		$CollisionShape2DBoom.set_deferred("disabled", false)
	$LifeTime.stop()
	play_death_sound()
	show_boom()
